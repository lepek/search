...
RSpec.describe 'FilterCollection' do
  ...
  before(:all) do
    @data_set = DataSet.instance.data_set(Rails.root.join('spec', 'fixtures', 'data.json'))
  end
  ...
  it "should return one matching when searching by a word" do
    ...
    @data_set.each do |data|
      results << data if filter_collection.apply(data)
    end
  ...
  it "should return one matching when searching by a phrase" do
    ...
    expect(results.count).to eq(1)
    expect(results.first['Designed by'].downcase).to include(search_string.gsub('"',''))
  end
  ...
  it 'should match microsoft scripting languages in the most relevant results' do
    ...
    results = []
    @data_set.each do |data|
      if filter_collection.apply(data)
        results[filter_collection.relevance] ||= Array.new
        results[filter_collection.relevance] << data
      end
    end
